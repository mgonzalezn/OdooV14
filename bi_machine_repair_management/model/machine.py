...
class AccountAnalyticLine(models.Model):      
    ...
    def create(self,values):
        ...
        timesheet_id = values.get('timesheet_id') or self.env['machine.diagnosys'].search([['machine_repair_id','=',machine_repair_id]],limit=1)
        machine_workorder_id = values.get('timesheet_workorder_id') or self.env['machine.workorder'].search([['machine_repair_id','=',machine_repair_id]],limit=1)

        if machine_repair_id:
            timesheets = self.env['account.analytic.line'].sudo().search([['machine_repair_timesheet_id','=',values.get('machine_repair_timesheet_id')]])
            if timesheet_id:
